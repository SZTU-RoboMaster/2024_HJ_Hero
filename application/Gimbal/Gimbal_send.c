...
void Send_Chassis_Speed(int16_t ch1, int16_t ch0, int16_t ch2, uint8_t s_l, uint8_t fire_lock){
    uint32_t send_mail_box;
    uint8_t Send_data[8];
    union ctrl_ch temp;
    tx_message.StdId = 0x110;   //110
...
void Send_Keyboard(int32_t W, int32_t A, int32_t S, int32_t D, int32_t V, int32_t Mouse_l) {
    uint32_t send_mail_box;
    uint8_t Send_data[8];
    ...
    Send_data[4] = (uint8_t)(V & 0xFF);
    Send_data[5] = (uint8_t)(Mouse_l & 0xFF);
    HAL_CAN_AddTxMessage(&hcan1, &tx_message, Send_data, &send_mail_box);
}
